...
def read_bis_authority(path: Path, expected_run_id: str):
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        required = {
            "run_id", "filename", "source", "source_capture_order",
            "slot", "equipment_id", "attack", "lock_status",
            "first_substat_type", "first_substat_value", "ordered_substats_json",
        }
    ...
    if not rows:
        raise ValueError(f"{path.name} has no trusted equipment rows")

    run_ids = {str(r.get("run_id", "")).strip() for r in rows}
    if run_ids != {expected_run_id}:
        raise ValueError(
            f"{path.name} RUN_ID mismatch: expected {expected_run_id}, found {sorted(run_ids)}"
        )

    filenames = [str(r.get("filename", "")).strip() for r in rows]
    if len(filenames) != len(set(filenames)):
        raise ValueError(f"{path.name} contains duplicate screenshot filenames")

    bag_orders = []
    for r in rows:
        if str(r.get("source", "")).strip() != "bag":
            continue
        try:
            order = int(r.get("source_capture_order", ""))
        except (TypeError, ValueError):
            raise ValueError(
                f"{path.name} has invalid BAG source_capture_order for {r.get('filename')}"
            )
        if order <= 0:
            raise ValueError(
                f"{path.name} has non-positive BAG source_capture_order for {r.get('filename')}"
            )
        bag_orders.append(order)

    if len(bag_orders) != len(set(bag_orders)):
        raise ValueError(f"{path.name} contains duplicate BAG source_capture_order values")

    return rows


def validate_authority_against_lock_rows(authority_rows, ocr_rows):
    """Validate same-run item identity while preserving OCR row-order authority."""
    lock_by_filename = {str(r["filename"]): r for r in ocr_rows}
    auth_by_filename = {str(r.get("filename", "")).strip(): r for r in authority_rows}

    missing = sorted(set(lock_by_filename) - set(auth_by_filename))
    extra = sorted(set(auth_by_filename) - set(lock_by_filename))
    if missing or extra:
        msg = []
        if missing:
            msg.append("missing authority rows: " + ", ".join(missing[:10]))
        if extra:
            msg.append("unexpected authority rows: " + ", ".join(extra[:10]))
        raise ValueError("; ".join(msg))

    for filename, lock_row in lock_by_filename.items():
        a = auth_by_filename[filename]
        try:
            aid = int(a.get("equipment_id", ""))
            aattack = int(a.get("attack", ""))
        except (TypeError, ValueError):
            raise ValueError(f"Invalid authority ID/Attack for {filename}")

        if str(a.get("source", "")).strip() != str(lock_row["source"]):
            raise ValueError(f"Authority source mismatch for {filename}")
        if str(a.get("slot", "")).strip() != str(lock_row["slot"]):
            raise ValueError(f"Authority slot mismatch for {filename}")
        if aid != int(lock_row["id"]):
            raise ValueError(f"Authority equipment_id mismatch for {filename}")
        if aattack != int(lock_row["signature"][1]):
            raise ValueError(f"Authority Attack mismatch for {filename}")

        try:
            stats = json.loads(a.get("ordered_substats_json") or "[]")
        except json.JSONDecodeError as exc:
            raise ValueError(f"Invalid ordered_substats_json for {filename}: {exc}")
        if not isinstance(stats, list):
            raise ValueError(f"ordered_substats_json is not a list for {filename}")

        first_type = str(a.get("first_substat_type", "") or "")
        first_value = str(a.get("first_substat_value", "") or "")
        if stats:
            first = stats[0] if isinstance(stats[0], dict) else {}
            expected_type = str(first.get("type") or "")
            expected_value = first.get("value")
            if isinstance(expected_value, float) and expected_value.is_integer():
                expected_value_text = str(int(expected_value))
            else:
                expected_value_text = "" if expected_value is None else str(expected_value)
            if first_type != expected_type or first_value != expected_value_text:
                raise ValueError(
                    f"First-substat authority mismatch for {filename}: "
                    f"columns=({first_type},{first_value}) JSON=({expected_type},{expected_value_text})"
                )
        elif first_type or first_value:
            raise ValueError(f"First-substat columns populated but ordered list empty for {filename}")



def main() -> int:
    base = Path(r"C:\MapleProjects\MapleOCR")
    ...
    authority_csv = results / f"bis_report_authority_{VERSION}.csv"
    authority_readme = results / f"BIS_REPORT_AUTHORITY_{VERSION}.txt"
    run_id_file = results / "RUN_ID.txt"
    out_zip = base / "BIS_stats.zip"

    required = [
        mapleexport, lock_status, maplelocked, review_csv, manifest, capture_csv,
        authority_csv, authority_readme, run_id_file
    ]
    ...
    try:
        ...
        authority_rows = read_bis_authority(authority_csv, lock_run_id)
        validate_authority_against_lock_rows(authority_rows, ocr_rows)
        ...
        members = [
            ...
            (authority_csv, authority_csv.name),
            (authority_readme, authority_readme.name),
        ...
        print(f"BIS report authority rows: {len(authority_rows)}")
        print("BIS authority: OCR first-substat row order + BAG source_capture_order validated")
